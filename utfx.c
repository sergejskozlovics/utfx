...
{
  return UTF8Processor(source, 0, NULL, UTFX_MAX_SIZE, ENCODING_KEEP_CURRENT, 0);
}
...
{
  return UTF16Processor(source, 0, NULL, UTFX_MAX_SIZE, ENCODING_KEEP_CURRENT, 0)/sizeof(WCHAR);
}
...
{
  return UTF32Processor(source, 0, NULL, UTFX_MAX_SIZE, ENCODING_KEEP_CURRENT, 0)/sizeof(LCHAR);
}
...
FILE *fopen_utf8(const char *utf8FileName, const char *mode)
{
  ...
  n1 = UTF8ToUTF16(utf8FileName, 0, NULL, UTFX_MAX_SIZE, 1);
  if (errno != 0)
    return NULL;
  n2 = UTF8ToUTF16(mode, 0, NULL, UTFX_MAX_SIZE, 1);
  if (errno != 0)
    return NULL;
  w1 = malloc(n1);
  w2 = malloc(n2);
...
int mkdir_utf8(const char *utf8Path)
{
  ...
  int retVal = 0;
  errno = 0;
  n = UTF8ToUTF16(utf8Path, 0, NULL, UTFX_MAX_SIZE, 1);
  if (errno != 0)
    return -1;
  w = malloc(n);
  if (w == NULL) {
...
char *getcwd_utf8(char *buf, size_t buflen)
{
  ...
  n = UTF16ToStandardUTF8(wbuf, 0, NULL, UTFX_MAX_SIZE, 1);
  if (errno != 0) {
    free(wbuf);
    return NULL;
  }
...
void *opendir_utf8(const char *utf8DirName)
{
  ...
  n = UTF8ToUTF16(utf8DirName, 0, NULL, UTFX_MAX_SIZE, 1);
  if (errno != 0)
    return NULL;
...
char *get_d_name_utf8(void *hOpenDir)
{
  ...
  n = UTF16ToStandardUTF8(h->e->d_name, 0, NULL, UTFX_MAX_SIZE, 1);
  if (errno != 0)
    return NULL;
  h->s = malloc(n);
  if (h->s == NULL)
...
char *getenv_utf8_malloc(const char *utf8name) // the result (if not NULL) must be freed by free()
{
  ...
  wchar_t *value = NULL;
  char* retVal = NULL;
  n = UTF8ToUTF16(utf8name, 0, NULL, UTFX_MAX_SIZE, 1);
  if (errno != 0)
    return NULL;
  w = malloc(n);
  if (w == NULL) {
  ...
  if (value == NULL)
    return NULL;
  n = UTF16ToStandardUTF8(value, 0, NULL, UTFX_MAX_SIZE, 1);
  if (errno != 0)
    return NULL;
  retVal = malloc(n);
  if (retVal == NULL)
